...
If you need to work with thousands of database records, consider using the `chunk` method. This method retrieves a small chunk of the results at a time and feeds each chunk into a `Closure` for processing. This method is very useful for writing [Artisan commands](https://laravel.com/docs/7.x/artisan) that process thousands of records. For example, let's work with the entire `users` table in chunks of 100 records at a time:
...
> ![](https://laravel.com/img/callouts/exclamation.min.svg)
>
> If you are adding a huge array of integer bindings to your query, the `whereIntegerInRaw` or `whereIntegerNotInRaw` methods may be used to greatly reduce your memory usage.
...
For more advanced `having` statements, see the [`havingRaw`](https://laravel.com/docs/7.x/queries#raw-methods) method.
...
> ![](https://laravel.com/img/callouts/exclamation.min.svg)
>
> Model events are not fired when using the `increment` and `decrement` methods.
