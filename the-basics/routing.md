...
{% hint style="warning" %}Использование прямого слэша поддерживаются только в пределах последнего сегмента маршрута.{% endhint %}
...
{% hint style="warning" %}Названия маршрутов всегда должны быть уникальными.{% endhint %}
...
{% hint style="info" %}Иногда нужно указать значения по умолчанию для URL-параметров в масштабе всего запроса, например, для текущей локали. Для этого можно использовать [метод `URL::defaults`](urls.md#znacheniya-po-umolchaniyu).{% endhint %}
...
{% hint style="warning" %}Для того чтобы убедиться, что ваши маршруты субдоменов доступны, необходимо зарегистрировать маршруты субдоменов до регистрации маршрутов корневых доменов. Это не позволит маршрутам корневого домена перезаписывать маршруты субдоменов, которые имеют один и тот же путь URI.
{% endhint %}
...
{% hint style="warning" %}Запасной маршрут всегда должен быть последним маршрутом, зарегистрированным в вашем приложении.
{% endhint %}
