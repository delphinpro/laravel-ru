...
Each of these methods may also be accessed via the `URL` [facade](https://laravel.com/docs/7.x/facades):
...
You will often be generating URLs using the primary key of [Eloquent models](https://laravel.com/docs/7.x/eloquent). For this reason, you may pass Eloquent models as parameter values. The `route` helper will automatically extract the model's primary key:
...
It is cumbersome to always pass the `locale` every time you call the `route` helper. So, you may use the `URL::defaults` method to define a default value for this parameter that will always be applied during the current request. You may wish to call this method from a [route middleware](https://laravel.com/docs/7.x/middleware#assigning-middleware-to-routes) so that you have access to the current request:
